...
public class PredictKeywordsChuck {
	...
	public static void main(String args[]){
		...
		for(IndividualVote vote : map.get("S270")){
		...
		while(!billList.isEmpty()){
			...
			for(int i = 0; i < 1 && !billList.isEmpty(); i++){
				testBills.add(billList.remove(r.nextInt(billList.size())));
			}
			
			doBillPrediction(testBills, map, "S270");
			allCorrect += correct;
			allTotal += total;
		}
	...
	public static void doBillPrediction(List<String> testBills, Map<String, List<IndividualVote>> map, String id){
		...
		for(IndividualVote vote : votes){
			...
			if(!testBills.contains(bill_id) || !isTest){
				...
				if(addTo != null){
					...
					Map<CharSequence, Integer> keywords = getBillKeywordCount(bill_id);
		...
		for(IndividualVote vote : votes){
			...
			if(testBills.contains(bill_id) || !isTest){
				if(vote.getVoteType() == IndividualVote.VOTE_YEA || vote.getVoteType() == IndividualVote.VOTE_NAY){
					total ++;
					Map<CharSequence, Integer> billKeywords = getBillKeywordCount(bill_id);
...
//					double naysim = cs.cosineSimilarity(billKeywords, nayKeywords);
	...
	public static Map<CharSequence, Integer> getBillKeywordCount(String bill_id){
